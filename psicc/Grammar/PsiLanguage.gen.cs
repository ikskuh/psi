...
sealed partial class PsiTokenizer : Tokenizer<PsiTokenType>
{
	...
	public PsiTokenizer(TextReader reader, string fileName, bool closeOnDispose) : base(reader, fileName, closeOnDispose)
	{
		...
		this.RegisterToken(PsiTokenType.REF, new Regex(@"\bref\b", RegexOptions.Compiled));

		this.RegisterToken(PsiTokenType.ARRAY, new Regex(@"\barray\b", RegexOptions.Compiled));
		...
		this.RegisterToken(PsiTokenType.CHAR, new Regex(@"\`(?:\\\`|.){1,10}\`", RegexOptions.Compiled));
